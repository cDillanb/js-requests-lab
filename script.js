...
sayHelloButton.addEventListener("mouseover", buttonHover);
...
sayHelloButton.addEventListener("mouseout", buttonUnHover);
     ...
     axios
       ...
       .then(res => {
         res.data.forEach((element) => {
           ...
           document.body.appendChild(p).textContent = element
         })
       })
...
const query = () => {
  axios
  .get(`http://localhost:3000/query-test/?chicken=yum&bruh=me`)
  .then((res) => {
    console.log(res.data)
  })
  .catch((err) => console.log(err))
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
      text came back as "you sent an empty query!"
    2: Send more than 1 query on the URL -- what happened? 
      and object gets console logged that says "You sent more than 1 query!" as a message value.
*/
